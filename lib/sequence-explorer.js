(function (global, factory) {
    ...
    function transitionYaxis(nameY, thousandsSeparator, percentage) {
      var trans = d3.transition().duration(1000);
      var myFrame = d3.select("g.sankeyFrame.single");
      ...
      myFrame.selectAll("g.node").filter(function (d) {
        return d.nameY === nameY;
      }).append("text").attr("class", "percentageLabel").attr("x", dx + "px").attr("y", dy + "px").text(getPercentage).transition(trans).style("opacity", 1);
    }
        ...
        selection.each(function () {
          ...
          allGraphs.forEach(function (col, colIndex) {
            col.forEach(function (container, rowIndex) {
              ...
              // sankey = d3.sankeySeq()
              sankey = sankeySeq().size([width, height]).sequence(sequence).categories(categories).nodeWidth(nodeWidth).nodePadding(nodePadding).nodes(graph.nodes).links(graph.links).correspondingCategories(chartAPI.correspondingCategories()
              ...
              d3.selectAll("g.axis.bottom > g.tick").on("click", function () {
                if (visMode === SINGLE) {
                  ...
                  var nameX = d3.select(this).classed("zoomed", true).datum();
              ...
              d3.selectAll("g.axis.left > g.tick").on("click", function () {
                if (visMode === SINGLE) {
                  var nameY = d3.select(this).classed("zoomed", true).datum();
                  if (percentages[0] === "%event" && corrCategories().indexOf(nameY) === -1) {
                    return;
                  }
                  d3.select(".nodeScaling").node().disabled = true;
                  d3.select(".labelOnOff").node().checked = false;
