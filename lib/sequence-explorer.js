(function (global, factory) {
  ...
  function addTransitionX(svg, transitionX, categories) {
    ...
    // hide links
    myFrame.select("g.links").transition(trans).style("opacity", 0);

    // hide not selected nodes
    var hideNodes = myFrame.selectAll("g.node").filter(function (d) {
      return d.nameX !== "meetup3";
    });
    hideNodes.transition(trans).style("opacity", 0);

    // hide y axis
    d3.select("g.axis.left").transition(trans).style("opacity", 0);
    ...
    var sumOfValues = updateNodes.filter(function (d, i) {
    ...
    var rectY = d3.scaleLinear().domain([0, sumOfValues]).range([0, frameY]);
    ...
    updateNodes.select("rect").transition(trans).attr("height", function (d) {
          ...
          col.forEach(function (container, rowIndex) {
            ...
            if (typeof transitionX !== "undefined") {
              d3.selectAll("g.axis.bottom > g.tick").on("click", function () {
                addTransitionX(svg, transitionX, categories);
              });
            }
          });
        });
