(function (global, factory) {
    ...
    function particles() {
      ...
      */
      publicAPI.init = function (_svgPaths, _canvasTL, _pathName, _myPath, _sequenceStart, _linkMax, _linkValue, _particlesMin, _particlesMax, _particlesSpeed) {
      ...
      publicAPI.start = function () {
        if (typeof myTimer === "undefined") {
          myTimer = {};myTimer[pathName] = d3.timer(tick);
        }
      ...
      function tick(elapsed) {
        ...
        allPaths.forEach(function (_path) {
          _path["particles"] = _path["particles"].filter(function (d) {
            if (d.current >= d.path.getTotalLength() && _path["startParticles"].indexOf(d.path.__data__.target.nameX) === -1) {
          ...
          _path["links"].filter(function (d) {
            return _path["startParticles"].indexOf(d.source.nameX) !== -1;
          }).forEach(function (d) {
    ...
    function sankeySeqExplorer (_myData) {
      ...
      function showRemoveParticles(_pathName) {
        if (!d3.select(this).node().checked) {
          ...
          myParticles = myParticles.init(classPaths.get(key), props.particleStart, _pathName, myPath, sequenceStart, mySankey.maxValue(), myPathValue, particlesMin, particlesMax, particlesSpeed).start();
        }
        return;
      }
