(function (global, factory) {
    ...
    function getColRowOfSingle(rootSelection) {
      var coord = rootSelection.select("g.sankeyFrame.single").attr("class").split(" ")[1]; // e.g. f0-0
    ...
    function transitionToSingle(parent, clickedElement, _trans) {
        var trans = typeof _trans !== "undefined" ? _trans : d3.transition().duration(1000);

        parent.selectAll("g.sankeyFrame").each(function () {
    ...
    function transitionToMultiples(parent, clickedElement) {
        var trans = d3.transition().duration(1000);

        parent.selectAll("g.sankeyFrame").classed("multiples", true).each(function () {
            if (this === clickedElement) {
                d3.select(this) // set class
                ...
                parent.selectAll(".axis").transition(trans).style("opacity", 0);

                parent.selectAll(".coverSankeySeq").transition(trans).style("opacity", 1);
            } else {
                d3.select(this).transition(trans).attr("transform", function (d) {
                    return d.multiples;
    ...
    function getPositionData(rootSelection, _elementsToLabel, _catSubset, height, width) {
      var labelSubset = [];
      var pData = {};
      ...
      function getLabelSize(_elementsToLabel, fontSize) {
        ...
        var dummy = rootSelection.select("svg").append("g").attr("class", "dummy l1").style("opacity", 0).selectAll("text").data(Object.keys(_elementsToLabel)).enter().append("text").style("font-size", fontSize + "px").style("font-family", "sans-serif").text(function (d) {
    ...
    function transitionXaxis(rootSelection, transitionX, nameX, nodeInfos, _thousandsSeparator) {
      var trans = d3.transition().duration(1000);
      var myFrame = rootSelection.select("g.sankeyFrame.single");
      ...
      var axis = rootSelection.selectAll("g.axis.left");
      ...
      var pData = getPositionData(rootSelection, elementsToLabel, catSubset, height, width);
      transitionAxis(pData, axis, catSubset, trans);
    }

    function transitionXaxisBack(rootSelection, _corrCategories, nameX, nodeInfos) {
      var myFrame = rootSelection.select("g.sankeyFrame.single");
      var trans = d3.transition().duration(1000);
    ...
    function transitionYaxis(rootSelection, nameY, thousandsSeparator, percentage, firstSequenceEle) {
      var trans = d3.transition().duration(1000);
      var myFrame = rootSelection.select("g.sankeyFrame.single");
      var dx = rootSelection.select("g.sankeyFrame.single rect.sankeyNode").attr("width") / 2; // position adjustment for text label
    ...
    function transitionYaxisBack(rootSelection) {
      var myFrame = rootSelection.select("g.sankeyFrame.single");
      var trans = d3.transition().duration(1000);
    ...
    function particles() {
      ...
      */
      publicAPI.init = function (rootSelection, _svgPaths, _canvasTL, _pathName, _myPath, _sequenceStart, _linkMax, _linkValue, _particleMin, _particleMax, _particleSpeed, _particleShape, _particleSize, _nodeWidth, _debugOn) {

        if (rootSelection.select("div.sankeyChart").selectAll("canvas").size() === 0) {
          initializeCanvas(rootSelection, _canvasTL, _linkMax, _particleMin, _particleMax, _particleSpeed, _particleShape, _particleSize, _nodeWidth, _debugOn);
        }
      ...
      publicAPI.stop = function (rootSelection, _pathName) {
        if (typeof _pathName === "undefined") {
          stopParticles(rootSelection);
        }
      ...
      function stopParticles(rootSelection) {
        myTimer.stop();
        myTimer = {};
        allPaths = [];
        rootSelection.select("div.sankeyChart canvas").remove();
      }

      function initializeCanvas(rootSelection, _canvasTranslate, _linkMax, _particleMin, _particleMax, _particleSpeed, _particleShape, _particleSize, _nodeWidth, _debugOn) {
        ...
        cw = rootSelection.select("div.sankeyChart svg").attr("width");
        ch = rootSelection.select("div.sankeyChart svg").attr("height");

        rootSelection.select("div.sankeyChart").insert("canvas", ":first-child").attr("width", cw).attr("height", ch).attr("class", "particles");

        if (debugOn) {
          console.log("canvas (width, height): (" + cw + ", " + ch + ")");
          console.log("canvas translate(x, y): (" + _canvasTranslate.x + ", " + _canvasTranslate.y + ")");
        }
        context = rootSelection.select("canvas").node().getContext("2d");
        context.clearRect(0, 0, cw, ch);
        context.translate(_canvasTranslate.x, _canvasTranslate.y);

        rootSelection.selectAll("g.sankeyFrame.single rect.sankeyNode").each(function (d) {
      ...
      function getIndex(_array) {
        ...
        for (var i = 0; i < _array.length; i++) {
          if (i === _array.length - 1) {
            index = i;break;
          } else if (num < _array[i].value / sumOfValues) {
            index = i;break;
          } else {
            num = num - _array[i].value / sumOfValues;
          }
        }
          ...
          _path["particles"].forEach(function (d) {
            if (d.current >= d.path.getTotalLength()) {
              if (d.needsNewLinkIndex) {
                var relevantPaths = _path["links"].filter(function (l) {
    ...
    function sankeySeqExplorer (_myData) {
      ...
      var file = void 0; // reference to data (embedded or in file)
      var nodeFile = void 0; // optional file with additional node infos
      var pathFile = void 0; // optional file with paths
      var nodeInfoKeys = void 0; // the key names of the additional node infos
      ...
      var valueName = void 0; // the column name of the frequency value
      ...
      var allGraphs = void 0; // data structure containing columns of rows of sankey input data;
      var tooltip = void 0;
      ...
      var props = void 0; // properties calculated in initialization function
      ...
      function chartAPI(selection) {
        selection.each(function (d) {
          ...
          if (typeof d !== "undefined") {
            ...
            if ((typeof _myData === "undefined" ? "undefined" : _typeof(_myData)) == "object") {
      ...
      function displaySankeyMenu(rootSelection) {
        ...
        if (!(allGraphs.cols === 1 && allGraphs.rows === 1)) {
          var div2 = divOm.append("span").attr("class", "span1").append("input").attr("class", "nodeScaling").attr("type", "checkbox").on("change", function (d) {
            updateScaling(rootSelection, d);
          });
        ...
        var div3 = divOm.append("span").attr("class", "span2").append("input").attr("class", "labelOnOff").attr("type", "checkbox").on("change", function () {
        ...
        var divNim = rootSelection.select("div.sankeyMenu").append("div").attr("class", "NodeInfoMenu");
      ...
      function displayPathsMenu(rootSelection) {
        ...
        if (rootSelection.select("div.PathsMenu").size() === 0) {
          ...
          var colRow = getColRowOfSingle(rootSelection);
          ...
          var divPm = rootSelection.select("div.sankeyMenu").append("div").attr("class", "PathsMenu").style("opacity", 0);
          ...
          .on("change", function (d) {
            showRemoveParticles.call(this, rootSelection, d);
          });
      ...
      function showRemoveParticles(rootSelection, _pathName) {
        if (!d3.select(this).node().checked) {
          ...
          if (rootSelection.select("g.sankeyFrame.single").size() === 0) {
            return;
          }
          ...
          myParticles = myParticles.init(rootSelection, classPaths.get(key), props.particleStart, _pathName, myPath, sequenceStart, mySankey.maxValue(), myPathValue, particleMin, particleMax, particleSpeed, particleShape, particleSize, nodeWidth, debugOn).start();
        }
        return;
      }
      ...
      function updateNodeInfo(rootSelection) {
        ...
        rootSelection.select("div.NodeInfoMenu").transition(trans).style("border-color", function () {
        ...
        rootSelection.selectAll("rect.sankeyNodeInfo").style("display", "inline") // reset style to inline if switch from none to other
        .transition(trans).attr("y", function (d) {
      ...
      function createChart(selection) {
        ...
        selection.each(function () {
          ...
          svg = rootSelection.append("div").attr("class", "sankeyChart").append("svg").attr("width", size[0]).attr("height", size[1]).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
          ...
          allGraphs.forEach(function (col, colIndex) {
            col.forEach(function (container, rowIndex) {
              ...
              rootSelection.select("div.sankeyChart > svg").on("click", function () {
                ...
                if (visMode === ZOOMX) {
                  ...
                  var nameX = rootSelection.select("g.zoomed").classed("zoomed", false).datum();
