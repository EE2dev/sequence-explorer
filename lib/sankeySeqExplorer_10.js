      ...
      allGraphs.forEach( function (col, colIndex) {
        col.forEach( function (container, rowIndex) {
          ...
          link = sankeyG.append("g").selectAll(".link")
            .data(graph.links)
          .enter().append("path")
            .attr("class", function(d) { return "link" + " lsx" + d.source.nameX.replace(" ", "_")
              + " lsy" + d.source.nameY.replace(" ", "_")
              + " ltx" + d.target.nameX.replace(" ", "_")
              + " lty" + d.target.nameY.replace(" ", "_"); })
            .attr("d", sankey.link())
            .style("stroke-width", function(d) { return Math.max(1, d.dy) + "px"; })
            ...
            .on("mousemove", function() {
              ...
              var index = tooltip.style("height").indexOf("px");
              if(index != -1)
          ...
          node.append("rect")
            .attr("class", function(d) {
              var res = "sankeyNode" + " nx" + d.nameX.replace(" ", "_") + " ny" + d.nameY.replace(" ", "_");
          ...
          .each(function(d) { // adjust height for lowest category in case text overlaps with x axis
            var transNode = getTranslation(d3.select(this.parentNode).attr("transform"))[1];
            var pyHeight = parseInt(d3.select(this).style("font-size"));              
            if (transNode + pyHeight > yOfBottomAxis) {
    ...
    dataGroups.forEach(function (dim2, col) {
      ...
      dim2.values.forEach(function(dim1, row) {
        ...
        graph.nodes.forEach(function (d, i) {
          ...
          } else if (allGraphs.cols === 1 && allGraphs.rows !== 1) { // one additional dimension
